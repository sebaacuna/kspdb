...
class MuColliderBase:
    def __init__(self, type):
        self.type = type


class MuColliderMesh(MuColliderBase):
    def read(self, mu):
        self.isTrigger = 0
        if self.type:
...
class MuColliderSphere(MuColliderBase):
    def read(self, mu):
        self.isTrigger = 0
        if self.type:
            self.isTrigger = mu.read_byte()
...
class MuColliderCapsule(MuColliderBase):
    def read(self, mu):
        self.isTrigger = 0
        if self.type:
            self.isTrigger = mu.read_byte()
...
class MuColliderBox(MuColliderBase):
    def read(self, mu):
        self.isTrigger = 0
        if self.type:
            self.isTrigger = mu.read_byte()
...
class MuColliderWheel(MuColliderBase):
    def __init__(self):
        MuColliderBase.__init__(self, 0)
